...
@section('sub-title','Barangay Health Certificate')
            ...
            <div class="row p-2">
                  ...
                  @forelse($brgyCertificates as $brgyCertificate)
                    <div class="col-md-6">
                          <div class="card">
                            <div class="card-body">
                              <h5 class="card-title">Barangay Health Certificate</h5>
...
<script> 
  $(document).on('click', '#create_record', function(){
      ...
      $('.modal-title').text('Add Barangay Health Certificate');
  ...
  $('#myForm').on('submit', function(event){
    ...
    $('.form-control').removeClass('is-invalid')
    var action_url = "{{ route('resident.barangay_health_certificate.store') }}";
    var type = "POST";

    if($('#action').val() == 'Edit'){
        var id = $('#hidden_id').val();
        action_url = "barangay_health_certificate/" + id;
        type = "PUT";
    }
  ...
  $(document).on('click', '.edit', function(){
      $('#formModal').modal('show');
      $('.modal-title').text('Edit Barangay Health Certificate');
      $('#myForm')[0].reset();
      $('.form-control').removeClass('is-invalid')
      ...
      $.ajax({
          url :"/resident/barangay_health_certificate/"+id+"/edit",
          dataType:"json",
          beforeSend:function(){
              $("#action_button").attr("disabled", true);
              $("#action_button").attr("value", "Loading..");
  ...
  $.confirm({
      ...
      buttons: {
          confirm: {
              ...
              action: function(){
                  return $.ajax({
                      url:"barangay_health_certificate/"+id,
                      method:'DELETE',
                      data: {
                          _token: '{!! csrf_token() !!}',
                      },
